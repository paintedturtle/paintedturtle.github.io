...
(function() {
  var Location_Information, Name_And_Address, addCashSavingsTo, addLocationInformationTo, addModelAndSerialNumberTo, addNameAndAddressTo, renderDocumentAttributes, renderKijijiAttributes, renderReverbAttributes, renderTextForKijiji, renderTextForReverb, renderTitleForKijiji, renderTitleForReverb;
  ...
  addCashSavingsTo = function(paragraphs, output) {
    var indexOfContactInfo;
    if (output == null) {
      output = [];
    }
    ...
    if (paragraphs[indexOfContactInfo]) {
      output = output.concat(paragraphs.slice(0, indexOfContactInfo));
      output = output.concat(["Save $1 if you pay with cash instead of purchasing through Reverb."]);
      return output = output.concat(paragraphs.slice(indexOfContactInfo, paragraphs.length));
    } else {
      return output = output.concat(paragraphs);
    }
  };
